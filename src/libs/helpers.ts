...
export const isDefined = <T>(argument: T | null | undefined): argument is T =>
  argument !== null && argument !== undefined;

export const isEmptyObject = <T extends object>(obj: T) =>
...
export const getRandomString = (limit: number) =>
...
export const clearObjectByTargetKeys = <B>(
  requiredFields: string[],
  body: B,
) => {
  const clearRequiredFields = requiredFields.map((f) => f.replace('?', ''));
  const decomposed = Object.entries(body as any).filter((e) =>
    clearRequiredFields.includes(e[0]),
  );
